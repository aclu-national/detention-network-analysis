...
## Purpose
The immigration detention system comprises a complex network of facilities that hold individuals under custody. Understanding the movement of individuals between these facilities is critical for holding them accountable - particularly as many are forced into a ["Blackhole"](https://www.aclu.org/documents/inside-the-black-hole) of detention. This research leverages network analysis to explore detainee "movements", defined as changes in facility bookings within a single detention episode, and transfers.
...
- [`location_input/`](/location_input): This folder holds the data used in `location_analysis.R` in three folders `monthly`, `yearly`, and `lookup`. 
...
### States
We merge the cleaned data with states by creating a combined look-up sheet using data from the [Deportation Data Project](https://deportationdata.org/data/reports.html#detention-facility-list), [The Vera Institute](https://github.com/vera-institute/ice-detention-trends/blob/main/metadata/facilities.csv), [The Marshall Project](https://github.com/themarshallproject/dhs_immigration_detention/blob/master/locations.csv), [TRAC](https://tracreports.org/immigration/reports/370/include/table3.html), and [ICE/Talton](https://www.ice.gov/doclib/detention/taltonFacilities.pdf). The data can be find in [`location_input/`](/location_input). After combining the data, we filtered out all detention facility codes that had more than one distinct state associated with them. You can find the lookup table [here](/output/code_lookup.csv).
